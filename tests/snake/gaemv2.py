...
class snake(object):
	...
	def moveself(self):
		global totalapples, tiks, field, score, appleposition, rows, columns
		...
		for n in range(-1, 2):
			for x in range(-1, 2):
				try:
					proximity.append(field[self.pos[0] + n][self.pos[1] + x])
				except:
					proximity.append(9)
		...
		if predictions[0] >= predictions[1] and predictions[0] > predictions[2] and predictions[0] > \
				predictions[3]:
			# left
			if self.dirnx == 1 and self.dirny == 0:
				pass
				...
				self.dirnx = -1
				self.dirny = 0
		elif predictions[1] > predictions[0] and predictions[1] > predictions[2] and predictions[1] > \
				predictions[3]:
			# right
			if self.dirnx == -1 and self.dirny == 0:
				pass
				...
				self.dirnx = 1
				self.dirny = 0
		elif predictions[2] > predictions[1] and predictions[2] > predictions[0] and predictions[2] >= \
				predictions[3]:
			#  up
			if self.dirnx == 0 and self.dirny == -1:
				pass
				...
				self.dirny = 1
				self.dirnx = 0
		elif predictions[3] > predictions[1] and predictions[3] > predictions[2] and predictions[3] > \
				predictions[0]:
			# down
			if self.dirnx == 0 and self.dirny == 1:
				pass
		...
		def onetarget(tdirnx, tdirny):
			cxold = self.pos[0]
			cyold = self.pos[1]
			tnewpos = (cxold + tdirnx, cyold + tdirny)
			cx = tnewpos[0]
			cy = tnewpos[1]
			reward =0
			if cx not in range(len(field)) or cy not in range(len(field[0])):  # crashing into walls
				...
				stepsold = abs(appleposition[0] - cxold) + abs(appleposition[1] - cyold)
				if(steps<stepsold):
					reward = 1
				else:
					reward = 0
		...
		d = "longth: " + str(self.length) + "  high: " + str(highscore) + "  avgcost: " + str(
			self.snaykbrain.avgcost)
